...
let item_text: Text;
let item_image: Item;
const ItemStackHelperClass = Reflection.getClass(
...
function countItems() {
  ...
  let count = 0;
  inv.getItems().forEach(
    JavaWrapper.methodToJava((inv_item) => {
...
const heldItemChangeListener = JsMacros.on(
  "HeldItemChange",
...
const itemCountCommand = Chat.getCommandManager()
  ...
  .executes(
    JavaWrapper.methodToJava((context) => {
      item = context.getArg("item") as ItemStackHelper;
      item_image.setItem(item);
      config.item = item.getItemId();
      FS.open("item_count.json").write(JSON.stringify(config));
      countItems();
