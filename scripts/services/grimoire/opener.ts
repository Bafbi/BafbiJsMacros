{
  ...
  function isClosedGrimoire(item: ItemStackHelper): boolean {
    return (
  ...
  function getClosedGrimoiresSlots(inventory: Inventory): number[] {
    const grimoiresSlots: number[] = [];
  ...
  const openContainerListener = JsMacros.on(
    ...
    JavaWrapper.methodToJava((event) => {
      ...
      screen.addText(
        "Closed Grimoires: " + grimoiresSlots.length,
        screen.getWidth() - 20 - 100,
        screen.getHeight() - 40,
        0xffffff,
        false,
      );
